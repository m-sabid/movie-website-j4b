...
const UpdateForm = ({ data, onSubmit, onCancel, fields }) => {
  ...
  return (
    <form className="rounded-md w-full p-4" onSubmit={handleSubmit(handleFormSubmit)} style={{backgroundColor:colors.mo_primary}}>
      <div className="grid grid-cols-2 gap-4">
        {fields.map((field, index) => (
          <div key={index} className="mb-4 col-span-2 md:col-span-2">
            <label htmlFor={field.name} className="block text-white font-bold">
              {field.label}
            </label>
            <input
